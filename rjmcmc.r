...
### Probabilité des mouvements Death et Birth

Dk = function(k,lambda,kmax) {ifelse((k==1||k>kmax),0,0.5*min(1,dpois(k-1,lambda)/dpois(k,lambda)))} 
Bk = function(k,lambda,kmax) {ifelse((k==1||k>kmax),0,0.5*min(1,dpois(k+1,lambda)/dpois(k,lambda)))} 

### Fonction qui génère d'une loi normale tronquée sur [a,b]
...
### Fonction qui calcule le mode d'une série de données
...
#################################################################
### Fonction qui sert à fusionner deux régions nucléosomales ####
#################################################################
...
#################################################################
### Fonction qui sert à subdiviser une région nucléosomale ######
#################################################################
    ...
    if (k[i-1]==1) {
      ...
      u<-runif(1)
         ...
         if (compteur==1000) {rhob[i] <- 0}
          ...
          if (j==1) {qalloc <- 1/(mu[i-1,j]-min(y))}       #densité de mutilde[i,j] 
      ...
      if (u<= Dk(k[i-1],lambda,kmax)) {
        ...
        ktilde[i]<-k[i-1]-1
         compteur=1
          ...
          if (j==1) {qalloc <- 1/(mu[i-1,j+1]-min(y))}  #densité de mutilde[i,j] 
