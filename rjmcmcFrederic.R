RJMCMC = function(yf, yr, niter, kmax, lambda, ecartmin, ecartmax, minReads)
{
  ...
  tau      <- 1 / ((max.y - min.y) ^ 2)
  E        <- (max.y + min.y) / 2
  priormu.const  <- priormu.init(kmax, max.y, min.y)
    ...
    if (k[i-1]==1) {
      ...
      if (u<=0.5) {
        ...
        if (compteur==1000) {rhob[i] <- 0}
          ...
          #for (m in 1:k[i-1]) { #k=1
          Kaf[,1] <- (wtilde[i,1] * (1/sqrt(sigmaftilde[i,1])) *
                      dt((yf - mutilde[i,1] + deltatilde[i,1]/2) / sqrt(sigmaftilde[i,1]),
                         dltilde[i,1]))
          Kbf[,1] <- (w[i-1,1]*(1/sqrt(sigmaf[i-1,1])) *
                      dt((yf-mu[i-1,1]+delta[i-1,1]/2)/sqrt(sigmaf[i-1,1]),
                         dl[i-1,1]))
          #}
          Kaf[,ktilde[i]] <- (wtilde[i,ktilde[i]]*(1/sqrt(sigmaftilde[i,ktilde[i]]))*
                              dt((yf-mutilde[i,ktilde[i]]+deltatilde[i,ktilde[i]]/2)/sqrt(sigmaftilde[i,ktilde[i]]),
                                 dltilde[i,ktilde[i]]))
          ...
          #}
          if (ktilde[i] >= 2) {
            Y1f[1:nf] <- log(rowSums(Kaf[1:nf,1:ktilde[i]]))
          } else { Y1f[1:nf] <- log(Kaf[1:nf, 1]) }
          #Y2f[1:nf] <- sapply(Kbf[1:nr, 1], log)
          Y2f[1:nf] <- log(Kbf[1:nf, 1])
          
          #for (m in 1:k[i-1]) { # Special case k=1
          Kar[,1] <- (wtilde[i,1]*(1/sqrt(sigmartilde[i,1])) *
                      dt((yr-mutilde[i,1]-deltatilde[i,1]/2)/sqrt(sigmartilde[i,1]),
                         dltilde[i,1]))
            Kbr[,1] <- (w[i-1,1]*(1/sqrt(sigmar[i-1,1]))*dt((yr-mu[i-1,1]-delta[i-1,1]/2)/sqrt(sigmar[i-1,1]),dl[i-1,1]))
          #}
          Kar[,ktilde[i]] <- (wtilde[i,ktilde[i]]*(1/sqrt(sigmartilde[i,ktilde[i]]))*dt(( yr -mutilde[i,ktilde[i]]-deltatilde[i,ktilde[i]]/2)/sqrt(sigmartilde[i,ktilde[i]]),dltilde[i,ktilde[i]]))
          ...
          #}
          if (ktilde[i] >=2) {
            Y1r[1:nr] <- log(rowSums(Kar[1:nr, 1:ktilde[i]]))
          } else { Y1r[1:nr] <- log(Kar[1:nr, 1]) }
          # Specific case since we are in the special case k==1
          #Y2r[1:nr] <- apply(Kbr[1:nr, 1:k[i-1]    ], MARGIN=1, function(x){log(sum(x))})
          #Y2r[1:nr] <- sapply(Kbr[1:nr, 1], log)
          Y2r[1:nr] <- log(Kbr[1:nr, 1])
          ...
          if (j==1) {qalloc=1/(mu[i-1,j]-min.y)}       #density of  mutilde[i,j] 
          else {qalloc<-1/(mu[i-1,j]-mu[i-1,j-1])}      
          
          rap.priormu = (priormu(mutilde[i,1:ktilde[i]], priormu.const, tau, E) /
                         priormu(mu[i-1,1:k[i-1]], priormu.const, tau, E))
        ...
        v=runif(1)      #Accept/reject Birth move  
      ...
      } ### end of B move in case k[i-1]=1 
      
      else {
        
        ##Metropolis-Hastings move for k[i-1]=1
        ktilde[i] <- k[i-1]
        compteur  <- 1
        repeat {                                      
          j <- sample(1:k[i-1],1)    # sample(1, 1)
        ...
        if (compteur==1000) {rhomh[i] <- 0}
          ...
          if (k[i-1] >=2) {
            Y2f[1:nf] <- log(rowSums(Kbf[1:nf, 1:k[i-1]]))
          } else { Y2f[1:nf] <- log(Kbf[1:nf,1]) }
      ...
      if (u<= Dk(k[i-1],lambda,kmax)) {
        ...
        ktilde[i]<-k[i-1]-1
         compteur=1
          ...
          Kbf[,k[i-1]]<- (w[i-1,k[i-1]]*(1/sqrt(sigmaf[i-1,k[i-1]]))*dt(( yf -mu[i-1,k[i-1]]+delta[i-1,k[i-1]]/2)/sqrt(sigmaf[i-1,k[i-1]]),dltilde[i,k[i-1]]))
          ...
          Kbr[,k[i-1]]<- (w[i-1,k[i-1]]*(1/sqrt(sigmar[i-1,k[i-1]]))*dt(( yr -mu[i-1,k[i-1]]-delta[i-1,k[i-1]]/2)/sqrt(sigmar[i-1,k[i-1]]),dl[i-1,k[i-1]]))
          ...
          if (k[i-1] >= 2){
            Y2r[1:nr] <- log(rowSums(Kbr[1:nr, 1:k[i-1]]))
          } else { Y2r[1:nr] <- log(Kbr[1:nr, 1]) }
          ...
          if (j==1) {
            qalloc <- 1/(mu[i-1,j+1]-min.y)  #density of mutilde[i,j] 
          } else {
            if (j==k[i-1]) {
        ...
        v = runif(1)      #Accept/Reject Death move  
        ...
        if (u<= (Dk(k[i-1],lambda,kmax)+Bk(k[i-1],lambda,kmax))) {
          ...
          if (compteur==1000) {rhob[i] <- 0}
            ...
            Kaf[,ktilde[i]]<- (wtilde[i,ktilde[i]]*(1/sqrt(sigmaftilde[i,ktilde[i]]))*dt(( yf -mutilde[i,ktilde[i]]+deltatilde[i,ktilde[i]]/2)/sqrt(sigmaftilde[i,ktilde[i]]),dltilde[i,ktilde[i]]))
            ...
            if (j==1) {
              qalloc=1/(mu[i-1,j]-min.y)        #density of mutilde[i,j] 
            } else {
  ...
  if (niter>=20000)
    {
      k      <-k[10000:niter]
      mu     <- mu[10000:niter, ]
      sigmaf <- sigmaf[10000:niter, ]
      sigmar <- sigmar[10000:niter, ]
      delta  <- delta[10000:niter, ]
      dl     <- dl[10000:niter, ]
      w      <- w[10000:niter, ]
    }
