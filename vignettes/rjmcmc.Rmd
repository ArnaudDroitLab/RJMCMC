...
A synthetic nucleosome sample containing 200 nucleosomes (150 nucleosomes 
well-positioned + 50 fuzzy nucleosomes) has been created using the 
...
val.num       <- 200  ### Number of well-positioned nucleosomes
...
analyzed region into candidate regions to accelerate the analysis. 

Moreover, it is mandatory to analyse each chromosome separatly since the 
RJMCMC algorithm can only analyze one chromosome at the time.
...
The RJMCMC algorithm must be run on each candidate region. As an example, the 
